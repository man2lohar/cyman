...
(function initMasterMulti() {
  ...
  function displayFilteredData(csv, blockLabels) {
    ...
    /* ── Accumulators ── */
    let RoadWidthSum = 0, landAreaSum = 0;
    let GroundCoverageSum = 0;
    ParkingAreaSum = 0;

    /* Per-block height: blockLabel → max height Line length */
    const blockHeights = {};
    blockLabels.forEach(l => blockHeights[l] = 0);

    /* Per-block open spaces: blockLabel → {front, rear, side1, side2} */
    const blockOS = {};
    blockLabels.forEach(l => blockOS[l] = { front:0, rear:0, side1:0, side2:0 });

    rows.forEach(r => {
      const { name, colour, layer, length, linetype, lineweight, area, closed } = r;
      const colourNum = parseInt(colour, 10);

      /* Road Width — unchanged */
      if (name === 'Line' && colour === '230' && layer === 'Road' &&
          linetype === 'ByLayer' && lineweight === '0.15 mm')
        RoadWidthSum += length;

      /* Plot / Land Area — unchanged */
      if (name === 'Polyline' && colour === '240' && layer === 'Plot' &&
          linetype === 'PHANTOM2' && lineweight === '0.50 mm' && closed === '-1')
        landAreaSum += area;

      /* Parking Area — unchanged */
      if (name === 'Polyline' && layer === 'Parking_Area' &&
          lineweight === '0.15 mm' && closed === '-1')
        ParkingAreaSum += area;
      ...
      if (name === 'Line' && layer === htLayer && linetype === 'ByLayer') {
        const lwIdx = LW_LIST.indexOf(lineweight);
        if (lwIdx >= 0 && lwIdx < blockCount) {
          const lbl = blockLabels[lwIdx];
          blockHeights[lbl] = Math.max(blockHeights[lbl], length); // keep tallest line for this block
        }
      }

      /* ── OPEN SPACES: per-block, colour 53+i*1 per direction ──
         Front  = 53+i, Rear = 63+i, Side1 = 73+i, Side2 = 83+i */
      if (name === 'Line' && layer === osLayer &&
          linetype === 'ByLayer' && lineweight === 'ByLayer') {
        if (!isNaN(colourNum)) {
          blockLabels.forEach((lbl, i) => {
            if (colourNum === 53 + i) blockOS[lbl].front  += length;
            if (colourNum === 63 + i) blockOS[lbl].rear   += length;
            if (colourNum === 73 + i) blockOS[lbl].side1  += length;
            if (colourNum === 83 + i) blockOS[lbl].side2  += length;
          });
        }
      }
    });

    /* ── Max height across all blocks ── */
    const maxHeight = Math.max(...Object.values(blockHeights), 0);

    /* ── Populate static cells ── */
    set('roadwidth',           fmt(RoadWidthSum) + ' M.');
    set('pro-height',          fmt(maxHeight) + ' M.   (max across all blocks)');
    ...
    /* ── Inject per-block height breakdown table ── */
    _injectHeightTable(blockHeights, blockLabels);

    /* ── Inject per-block open spaces table — pass blockHeights directly ── */
  ...
  function calculateProposedFAR() {
    ...
    const totalPark  = parseFloat(localStorage.getItem('totalParkingArea')) || 0;
  ...
  function TreeCover() {
    const landArea   = parseFloat(document.getElementById('land-area')?.textContent.replace(/,/g, '').split(' ')[0]);
    const feesTotal  = parseFloat(document.getElementById('total-fees-area')?.textContent.replace(/,/g, '').split(' ')[0]);
    const treecover  = (0.0025 * feesTotal) * landArea / 100;
    const permPct    = 0.0025 * feesTotal;
    const cbloft     = 0.03 * feesTotal;
    set('per-tree', `${treecover.toFixed(3)} Sq. m. (${permPct.toFixed(3)}%)`);
  ...
  const minimumOpenSpaces = {
    ...
    industrial: [
      ...
      { height:'Above 21.5 M.', front:'20% of height or 6 M., whichever is more', side1:'20% of height or 6.5 M., whichever is more', side2:'20% of height or 6.5 M., whichever is more', rear:'20% of height or 10.0 M., whichever is more' },
    ],
    storage: [
      { height:'Up to 10.0 M.',           front:'12.5 M.', side1:'5.0 M.', side2:'4.0 M.',  rear:'4.0 M.' },
      { height:'Above 10.0 M. up to 21.5 M.', front:'12.5 M.', side1:'6.0 M.', side2:'6.5 M.', rear:'10.0 M.' },
