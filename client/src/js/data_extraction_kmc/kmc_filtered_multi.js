...
function _generateMasterFiltered(container, csv) {
  ...
  const gc = _mfTotalAreaForLayer(parsedData, 'Ground Coverage');
  const tr = _mfTotalAreaForLayer(parsedData, 'Terrace');
  if (parseFloat(gc) !== parseFloat(tr)) {
    _mfErrors.push(`${_mfErrCount++}. Total Area of Ground Coverage: ${gc} is not equal to Total Area of Terrace: ${tr}.`);
  }
  ...
  for (const layer in layerData) {
    ...
    layerData[layer].forEach(d => {
      ...
      if (layer === 'Lift') {
        ...
        if (lobby > 5) {
          _mfErrors.push(`${_mfErrCount++}. Lift Lobby Area exceeds limit of 5 (Value: ${lobby})`);
        }
        totals.tfa += parseFloat(d.totalFloorArea) || 0;
        totals.da  += lobby;
      } else if (SPECIAL_LAYERS.includes(layer)) {
